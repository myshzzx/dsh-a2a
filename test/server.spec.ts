...
describe('A2A server with a harness executor', () => {
  ...
  it('mints a session dir with no double dash for a UUID-style contextId', async () => {
    ...
    // basename, not split('/'): the session dir is joined with the host
    // separator, so a literal '/' split would return the whole path on Windows.
    const dir = basename(creates[0]?.meta?.cwd ?? '')
  ...
  it('runs every session in one shared directory when sharedCwd is on', async () => {
    ...
    try {
      ...
      for (const contextId of ['ctx-1', 'ctx-2']) {
        ...
        await executor.attachToWorkspace(String(sessionIdFor('test', contextId)), contextId)
      }
